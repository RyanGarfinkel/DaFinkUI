...
4. Register block example variants in `app/_docs/registry/blockExamples.tsx` as `blockExamples[slug]: BlockExample[]` (`id`, `title`, optional `description`, `usage`, `preview`, `slug`). The variant's source component lives in its own `src/blocks/{Name}/{Name}.tsx` file and **is** given its own entry in `blocks.ts`'s `blocks` array (with `hidden: true`) plus a matching entry in the CLI's `packages/cli/src/lib/blocksRegistry.ts`, so it's individually installable (`npx dafink-ui add {variant-slug}`) and directly linkable at `/blocks/{variant-slug}`, but `hidden: true` keeps it off the `/blocks` gallery grid and out of the parent's own listing: it's surfaced only through the parent block's Demos section. `BlockExample.slug` points at that hidden entry; `[slug]/page.tsx` looks it up via `getBlock(example.slug)` to render a small install command + "Also installs" line under each extra demo. This is a deliberate difference from `componentExamples.skeleton`-style sub-variants, which have no independent registry entry at all: block variants are real, standalone files, so they're worth making separately installable even though they're not separately listed.
...
`app/(docs)/theme/page.tsx` is a single merged page: there is no separate `/styles` route. It covers, in order: a live `ThemeToggle` demo plus its Installation snippet ("Light & dark mode"), the pre-built themes grid ("Color palettes", driven by `themes` from `@/src/themes`, each a color-only swap; radius/shadow/border come from the Style, not the Theme), the style cards (driven by `styles` from `@/src/styles`), how to apply a theme, how to author a custom theme, and the underlying CSS variable/token-build pipeline.
