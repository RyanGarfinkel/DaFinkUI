...
- `'use client';` on line 1, since blocks are interactive by nature
...
- Not exported from `src/index.ts`, since blocks are never `npm install`ed, only copied via the CLI or by hand
