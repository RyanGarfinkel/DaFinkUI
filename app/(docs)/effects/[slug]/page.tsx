...
import { effectExtraSections } from '@/app/_docs/registry/effectExtraSections';
import { EffectLivePreview } from '@/app/_docs/components/EffectLivePreview';
import { ComponentPreview } from '@/app/_docs/components/ComponentPreview';
import { effectExamples } from '@/app/_docs/registry/effectExamples';
import { getEffect, effects } from '@/app/_docs/registry/effects';
...
export const generateStaticParams = async () => {
  return effects.map((entry) => ({ slug: entry.slug }));
};

const EffectPage = async (
  {
    params,
  }: {
  ...
  const extraDemos = effectExamples[slug] ?? [];
  ...
  return (
    <div className='flex flex-col gap-10'>
      ...
      {/* Demo(s): a list of at least one (the primary live preview + its usage
          ...
          effectExamples. Every item shows its own Preview/Code toggle, so
      ...
      {/* Composition: only rendered for compound effects */}
      {entry.composition && (
        <section className='flex flex-col gap-3'>
          <h2 className='text-sm font-semibold text-text uppercase tracking-wide'>
            Composition
      ...
      {(effectExtraSections[slug] ?? []).map((section) => (
