...
| effect | `RevealEffect` | n/a | Default effect for child Reveal items that don't set their own. |
| once | `boolean` | n/a | Default `once` for child Reveal items that don't set their own. |
| children | `ReactNode` | required | `<Reveal>` elements (non-Reveal children render unchanged). |
