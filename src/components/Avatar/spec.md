...
| name | `string` | — | Full name used to derive fallback initials ("Ada Lovelace" → "AL") and the fallback's accessible label when `alt` is absent. |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| max | `number` | — | Maximum avatars shown before collapsing the rest into a "+N" indicator. |
