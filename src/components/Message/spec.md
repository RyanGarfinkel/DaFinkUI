...
| avatar | `ReactNode` | n/a | Optional avatar rendered beside the bubble (e.g. `<Avatar />`). Sits at the outer edge, bottom-aligned with the bubble via `items-end`. |
...
| children | `ReactNode` | n/a | The message content. A `MessageReactions` child is detected and extracted automatically: render it anywhere inside `Message`, it does not need to be last. |
...
| max | `number` | n/a | Maximum number of chips shown. Once exceeded, the remaining chips collapse into a `role="img"` "+N" indicator and the visible chips overlap into a stack, the same `max`/overflow pattern as `AvatarGroup`. Omit it to let every chip wrap onto its own row instead. |
| className | `string` | `''` | Additional CSS classes. |
| children | `ReactNode` | n/a | One or more `MessageReaction` chips. |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| icon | `ReactNode` | n/a | Decorative glyph: an emoji string or icon node. Always `aria-hidden`. |
| label | `ReactNode` | n/a | Visible text next to the icon: a count label, a footnote number (`"1"`), or a source name. |
| count | `number` | n/a | Aggregate count for a tapback-style reaction (e.g. 3 people reacted). When set, drives the auto-generated `aria-label` and is shown in place of `label` if `label` is omitted. |
...
| href | `string` | n/a | Renders the chip as a link (citation/source use case). Mutually exclusive with button-only native props. |
| className | `string` | `''` | Additional CSS classes. |
...
- The `max` overflow indicator is a `role="img"` span (not a button, since it triggers no action) with its own `aria-label` (e.g. `"2 more reactions"`, singular `"1 more reaction"`) so its count is announced even though the `+N` text itself is `aria-hidden`.
- The `active`/"yours" state is never communicated by color alone: it also changes the border, fills the chip solid, and adds a checkmark icon; the computed `aria-label` also says "including yours" outright.
