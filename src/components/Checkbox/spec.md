...
|------|------|---------|-------------|
| label | `string` | None | Text label rendered beside the checkbox. |
| id | `string` | None | HTML id for the input; links the label element via `htmlFor`. |
| checked | `boolean` | None | Controlled checked state. |
| defaultChecked | `boolean` | None | Uncontrolled initial checked state. |
| disabled | `boolean` | `false` | Disables the checkbox. |
| onChange | `ChangeEventHandler<HTMLInputElement>` | None | Native change event handler. |
