...
|--------------|------------------------------------------------------------------------|------------------|-------------------------------------------------------------------------------|
| name         | `string`                                                                | None             | The filename shown in the chip. Also used to infer the file-type icon and as the fallback `title` (full name on hover/truncation) if `title` isn't set. |
| href         | `string`                                                                | None             | Link target for the file. When set, the primary control renders as a real `<a>`. When omitted, it renders as a `<button type="button">` instead, so the primary action stays keyboard-reachable even without a navigable URL. |
| size         | `string`                                                                | None             | Pre-formatted file size (e.g. `"2.4 MB"`), shown as secondary muted text.    |
| type         | `string`                                                                | None             | Pre-formatted file type label (e.g. `"PDF"`), shown alongside `size` as secondary muted text (`"PDF · 2.4 MB"`). |
...
| thumbnail    | `string`                                                                | None             | An image URL rendered in place of the icon glyph. For an image-type attachment this defaults to `href` automatically: the image file is its own preview. For anything else (a PDF, say), there's no such shortcut: pass a pre-rendered cover/preview image explicitly, since a browser can't render a non-image file as an `<img>`. |
| onClick      | `(event: MouseEvent<HTMLAnchorElement \| HTMLButtonElement>) => void`  | None             | Fires when the primary control (link or button) is activated: the "open" action. Call `event.preventDefault()` here if you want to replace default link navigation with custom behavior (e.g. opening a preview in a new tab). |
...
| children  | `ReactNode` | None    | One or more `Attachment` elements.                                |
...
- The thumbnail `<img>` (when shown) has `alt=""`, since its meaning is entirely redundant with the visible filename text right next to it, so it's treated as decorative to avoid double-announcing the same information to screen readers.
