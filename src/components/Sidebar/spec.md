...
Collapse toggle button (`collapsible`): a circular button straddling the aside's right border. Position is controlled by `togglePosition`: `middle` (default, vertically centered — safest choice with no adjacent top bar), `top` (`top-11` — centers the button on the bottom edge of a standard `h-14` (56px) `TopNav`, so it sits right at the intersection of the sidebar's and top bar's borders), or `bottom` (near the footer). Rotates its chevron 180° and swaps its `aria-label` between "Collapse navigation" and "Expand navigation" on toggle. Width transitions between `width` and `collapsedWidth` via `motion-safe:transition-[width]`.
...
Use `Sidebar` for persistent vertical navigation in application layouts — dashboards, docs sites, settings panels. Pair with a `SidebarSection` per logical group of links. Use `SidebarDivider` to separate unrelated groups. Use `SidebarHeader` for a brand/logo slot and `SidebarFooter` for account-level actions (e.g. a user menu or logout link) pinned to the bottom. Add `collapsible` when the host layout benefits from a narrow icon-rail mode — pass an `icon` to the links you want to remain meaningful once collapsed.
